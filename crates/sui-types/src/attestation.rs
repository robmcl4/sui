...
impl fmt::Display for NitroError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            NitroError::UserDataMissing => write!(f, "UserDataMissing"),
...
/// and check the user_data is consistent with pk.
pub fn attestation_verify_inner(
    ...
    // Parse attestation into a valid cose sign1 object.
    let cose_sign1 = CoseSign1::from_bytes(attestation_bytes)?;
    cose_sign1.validate_header()?;
    ...
    let user_data = doc.clone().user_data.ok_or(NitroError::UserDataMissing)?;
    if user_data != pk {
...
impl AttestationDocument {
    /// Parse the payload of the attestation document. Adapted from https://github.com/EternisAI/remote-attestation-verifier/blob/main/src/lib.rs
    pub fn parse_payload(payload: &Vec<u8>) -> Result<AttestationDocument, NitroError> {
    ...
    fn validate_cert(&self, now: u64) -> Result<(), NitroError> {
        ...
        let root_cert = CertificateDer::from_pem_slice(ROOT_CERTIFICATE_PEM).map_err(|e| {
